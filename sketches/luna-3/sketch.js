...
class Vector {
  ...
  add(x, y) {
    if (arguments.length === 1) { this.x += x.x; this.y += x.y }
    else if (arguments.length === 2) { this.x += x; this.y += y }
    return this
  }
  sub(x, y) {
    if (arguments.length === 1) { this.x -= x.x; this.y -= x.y }
...
class Dot {
 ...
 update( dt) {
    ...
    let direction = dist === 0 ? new Vector(0, 0) : new Vector(dx / dist, dy / dist)
...
drawText(ctx) {
  ...
  draw(ctx) {
    ...
    ctx.fillRect(this.pos.x - this.mass, this.pos.y - this.mass, this.mass * 2, this.pos.y ? this.mass * 2 : this.mass * 2)
  }
}
...
class Rope {
  ...
  pin(index) { this.dots[index].pinned = true }
...
class Garland {
  ...
  create() {
    ...
    for (let i = 0; i < this.segments; i++) {
      this.sticks.push(new Stick(this.dots[i], this.dots[i + 1], this.lineWidth))
    }
  }
...
preloadImages([svgDefault, svgActive])
  .then(() => {
